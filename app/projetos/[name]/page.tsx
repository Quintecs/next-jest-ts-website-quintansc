import { PageStructuredData } from "@/components/site/structured-data";
...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  if (!project) notFound();
  return pageMetadata(`/projetos/${encodeURIComponent(project.name)}`, `${project.name} — ${project.tags.join(", ")}`, project.description);
}
...
export default async function ProjetoPage({ params }: Params) {
  ...
  return (
    <>
      <PageStructuredData
        path={`/projetos/${encodeURIComponent(project.name)}`}
        page={{ title: `${project.name} — ${project.tags.join(", ")}`, description: project.description }}
        mainEntity={{
          "@type": "CreativeWork",
          name: project.name,
          description: project.description,
          author: { "@id": siteUrl("/sobre#person") },
