  ...
  return (
    <div className="grid-surface relative rounded-2xl border border-edge bg-panel p-5 sm:p-8" aria-label="Da ideia à solução: estratégia, interface, integração e entrega">
      ...
      <div className="mx-auto max-w-xs">
        <div className="rounded-xl border border-primary-light/20 bg-primary/10 p-5"><div className="mb-3 flex items-center justify-between"><span className="font-mono text-xs text-accent">01 / ESTRATÉGIA</span><ArrowUpRight size={16} className="text-muted" /></div><p className="text-xl font-medium tracking-tight">Seu objetivo de negócio</p><p className="mt-2 text-sm text-muted">O ponto de partida de cada decisão.</p></div>
        <ArrowDown className="mx-auto my-4 text-muted-dark" size={20} />
        <div className="grid grid-cols-2 gap-3">{[{ icon: Globe2, title: "Interface", text: "Clara e acessível" }, { icon: Database, title: "Integração", text: "Tudo conectado" }].map(({ icon: Icon, title, text }) => <div key={title} className="rounded-xl border border-edge bg-background p-4"><Icon size={21} className="mb-5 text-primary-light" /><p className="font-medium">{title}</p><p className="mt-1 text-xs text-muted">{text}</p></div>)}</div>
        <ArrowDown className="mx-auto my-4 text-muted-dark" size={20} />
