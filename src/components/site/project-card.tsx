...
export default function ProjectCard({ project }: { project: Project | null }) {
  ...
  return (
    <Link href={`/projetos/${encodeURIComponent(project.name)}`} analyticsEvent={{ name: "project_click", properties: { project: project.name, destination: "details" } }} data-testid="projectCard" className="group flex h-full min-w-0 flex-col overflow-hidden rounded-xl border border-edge bg-panel transition-colors hover:border-muted-dark">
